...
// Dùng CHUNG project Supabase & bảng "orders" với dự án V3-KHBD (theo lựa chọn của
// người dùng) — nhờ vậy webhook payOS đã cấu hình sẵn cho V3-KHBD sẽ tự nhận diện và
// đánh dấu "paid" cho cả đơn hàng của app này, không cần đăng ký thêm webhook mới.
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Chỉ nhận deviceId + packageId từ client. KHÔNG nhận amount/orderCode từ client —
    // hai giá trị này do server tự quyết định, tránh việc client gửi sai số tiền.
    ...
    //    id tự tăng (bigserial) của bản ghi này sẽ dùng làm orderCode — đảm bảo không
    //    bao giờ trùng với bất kỳ đơn nào khác, kể cả đơn từ ứng dụng V3-KHBD dùng chung bảng.
    ...
    const description = `DE50 ${deviceId.replace(/-/g, '')}`.substring(0, 25);
    ...
    const result: any = await payosRes.json();
