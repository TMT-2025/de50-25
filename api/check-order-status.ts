...
// Cùng project/bảng Supabase với create-payment.ts (xem ghi chú ở đó).
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Bắt buộc khớp cả orderCode LẪN deviceId — tránh 1 thiết bị dò hỏi trạng thái đơn
    // hàng của thiết bị khác.
    ...
    if (order.claimed_at) {
      // Đã cộng lượt tải cho đơn này rồi (client có thể poll trùng vài nhịp) — báo lại
    ...
    // claimed_at còn NULL) để chống race condition khi client gọi 2 request gần như
    // đồng thời (ví dụ mở 2 tab) — chỉ 1 trong 2 request thắng và nhận được credits.
